...
| Функция | Описание |
| :--- | :--- |
| 📖 **Открытие документов** | Загрузка и чтение файлов `.docx` через Open XML SDK |
| 💾 **Сохранение документов** | Редактирование и сохранение с сохранением структуры Word |
| 🤖 **Суммаризация** | Краткое изложение содержания документа через AI |
| 🔄 **Перефразирование** | Улучшение стиля и грамматики текста через AI |
| 📚 **История запросов** | Локальное хранение истории обработки документов (LiteDB) |
| 🗄️ **Кэширование** | Автоматическое кэширование документов для ускорения работы |
| 🔒 **Приватность** | Вся обработка происходит локально, без отправки данных в интернет |
...
| **AI Inference** | LLamaSharp (Phi-3-mini-4k-instruct-q4.gguf) |
...
| AI Модель | Phi-3-mini-4k-instruct-q4.gguf | https://huggingface.co/microsoft/Phi-3-mini-4k-instruct-gguf |
...
| `Threads` | CPU Count | Количество потоков процессора |
...
│                   DocMind.Services                      │
│  ┌─────────────┐    ┌──────────────┐    ┌────────────┐ │
│  │ AiService   │    │DocumentService│   │  CacheSvc  │ │
│  │(LLamaSharp) │    │  (Open XML)  │    │  (LiteDB)  │ │
│  └─────────────┘    └──────────────┘    └────────────┘ │
└─────────────────────────────────────────────────────────┘
                          ↕
┌─────────────────────────────────────────────────────────┐
...
| Аспект | Реализация |
| :--- | :--- |
| **Данные** | Вся обработка локально, без отправки в облако |
| **Модель** | Локальный файл `.gguf`, не требует интернета |
| **Кэш** | LiteDB в `%LOCALAPPDATA%`, доступ только пользователю |
| **История** | Хранится локально, можно очистить через UI |
...
| :--- | :--- |
| **Только .docx** | Формат `.doc` (старый Word) не поддерживается |
| **Размер модели** | Phi-3-mini требует ~2.5 ГБ свободного места |
| **CPU инференс** | GPU не используется (GpuLayerCount = 0) |
| **Один запрос** | Обработка через SemaphoreSlim (очередь запросов) |
...
| **Автор** | NIKITA230009 |
| **Issues** | https://github.com/NIKITA230009/DocMindAISum/issues |
...
- [LLamaSharp](https://github.com/SciSharp/LLamaSharp) — .NET bindings для LLama
- [Microsoft Phi-3](https://huggingface.co/microsoft/Phi-3-mini-4k-instruct-gguf) — компактная AI модель
- [LiteDB](https://www.litedb.org/) — embedded NoSQL база данных
- [DocumentFormat.OpenXml](https://github.com/dotnet/Open-XML-SDK) — работа с Office документами
